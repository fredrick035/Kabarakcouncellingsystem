...
if(isset($_POST['update'])){
    ...
    if($decision=="Declined"){
      ...
      $dates=date('Y-m-d',strtotime($_POST['date'].'+2days'));
      //
      $query = "SELECT `dates` FROM `heap_sort` WHERE id='$id' Limit 1";
        $result = mysqli_query($db, $query) or die(mysqli_error($db));
        $rows = mysqli_num_rows($result);
        if ($rows ==$dates) {
          $dates=date('Y-m-d',strtotime($_POST['date'].'+3days'));}
            
      //
          $query = "INSERT INTO heap_sort (fname,dates) 
                VALUES('$sname', '$dates')";
                $results=mysqli_query($db, $query);
          header( "Refresh:2; url=view_booking.php");
         }
        else{
          $dates=date('Y-m-d',strtotime($_POST['date'].'+4days'));
          $query = "INSERT INTO heap_sort (fname,dates) 
                VALUES('$sname', '$dates')";
                $results=mysqli_query($db, $query);
          header( "Refresh:2; url=view_booking.php");
