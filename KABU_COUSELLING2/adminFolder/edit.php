...
<div class="split right">
  ...
  <label><b>Registration Number:</b></label><br>
		...
		<input type="time" placeholder="Set time" name="time" id="p2" >
...
//Sending to Cancellor
?>
<?php
if(isset($_POST['addtocancelor'])){
  $db= mysqli_connect("localhost","root","","kabarak_db");
    ...
    $decision=$_POST['decision'];
		$sql = "INSERT INTO booking_verification (fname, gender, contact,regnumber,dates,tim,texts,email,decision) VALUES ('$sname','$gender','$contact','$regnumber','$dates','$times','$texts','$email','$decision') ";
	if (mysqli_query($db, $sql)) 
	{
    header( "Refresh:2; url=reschdule.php?msg= You have sent to councelor  succesfully");
...
if(isset($_POST['submit'])){
  $db= mysqli_connect("localhost","root","","kabarak_db");
		...
		$texts=$_POST['texts'];
    $email=$_POST['email'];
		$sql = "UPDATE `booking` SET `fname`='$sname',`gender`='$gender',`contact`='$contact',`regnumber`='$regnumber',`dates`='$dates',`tim`='$times',`texts`='$texts',`email`='$email' WHERE id= $id ";
  ...
  '.$msg.'
    ...
    header( "Refresh:3; url=reschdule.php");
		//header( "Refresh:3; url=cover.php");
	} else{
    echo mysqli_error($db);
  }
...
<div class="split right">
  ...
  <label><b>Registration Number:</b></label><br>
		...
		<label><b>Time:</b></label><br>
		<input type="time" placeholder="Set time"value="<?php echo $row['tim']?>" name="time" id="p2" >
    ...
    <label><b>Message:</b></label><br>
<label><b>Accept/Declined</b></label><br>
...
<input type="radio" name="decision" value="Declined"<?php echo( $row['decision']=='Declined')?"checked":"";?>>Decline
      ...
      <button type="submit" name="addtocancelor" style="float:right">submit to Cancelor</button>
  </div>
</div>
		</div>
  </div>
