...
<h1 style="font-family:'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif ;">Your History</h1>
 <nav>
    ...
    <div class="dash"style="margin:40px;"><a href="studentDashbord.php">Dashboard</a></div>
</div>
 </nav>
...
$qry="SELECT * FROM `heap_sort` WHERE fname='$loggedUser'";
$run=$db-> query($qry);
if($run->num_rows>0){
    while($row=$run->fetch_assoc()){
        $id=$row['fname'];
...
<tr>
    <td><p>Your Session has been rescheduled on:</p></td>
    
</tr>
<tr>
<td><p><?php echo $row['dates']  ?></p>
<p> at            <?php echo $row['time']  ?></p>
</td>
</tr>
